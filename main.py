...
if target_event_id:
    ...
    try:
        ...
        if st.session_state.booking_step == 1:
            ...
            st.success(f"ご予約ありがとうございました！ご予約が確定しました。\n\n■ 対象イベント： {event_name}\n■ ご予約番号： {new_id} 番\n■ お席： {seat_display}")
                ...
                if active_events.empty:
                    ...
                    if event_res.empty:
                        ...
                        for _, row in event_res.iterrows():
                            label = f"予約番号: {row['予約ID']} 番 | {row['お名前']}様 | 計{row['人数']}名 | {row['座席番号']}席"
